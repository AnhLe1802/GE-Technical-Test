...
namespace AxieMixer.Unity
{
    public class AutoBlendAnimController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            var      ss       = renderer.sharedMaterial.GetTag("DisableBatching", false);
            var      ss1       = renderer.sharedMaterial.GetTag("DisableBatching", true);
            renderer.sharedMaterial.enableInstancing = false;
            renderer.sharedMaterial.SetOverrideTag("DisableBatching", "False");
        }
